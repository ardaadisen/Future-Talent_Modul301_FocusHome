...
import { initialAiParsedTask } from "../data/mockData";
...
export default function CreateTask({ onCreateManual, onConfirmAiTask }) {
  ...
  const parseWithAi = () => {
    setParsedTask({
      ...initialAiParsedTask,
      description: `Mock parse from input: "${nlInput}"`,
    });
    setEditTask(null);
  };
  ...
  return (
    <div className="page-grid">
      <div>
        <SectionHeader title="AI-Assisted Task Creation" subtitle="Mock parser for Week 4 frontend." />
        <AppCard title="Natural Language Input">
          ...
          <AppButton onClick={parseWithAi}>Parse with AI</AppButton>
        </AppCard>

        {parsedTask ? (
          <AppCard title="Mock Parsed Task Result">
          ...
          <AppCard title="Task Confirmation / Edit">
            ...
            <div className="field">
              ...
              <select
                ...
                onChange={(e) =>
                  setEditTask((prev) => ({ ...prev, durationMinutes: Number(e.target.value) }))
                }
              >
      ...
      <div>
        <SectionHeader title="Manual Task Creation" subtitle="Local state only, no backend call." />
        <AppCard title="Create Manual Task">
          ...
          <div className="field">
            ...
            <small className="muted">
              Total: {manualTask.durationSeconds} seconds ({manualTask.durationMinutes} minute view)
            </small>
          </div>
          <div className="field">
            <label>Difficulty</label>
          ...
          <AppButton
            onClick={() =>
              onCreateManual({
                ...manualTask,
                title: manualTask.title || "Untitled Manual Task",
              })
            }
          >
            Save Manual Task
